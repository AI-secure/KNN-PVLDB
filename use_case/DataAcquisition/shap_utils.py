...
def return_model(mode, **kwargs):
    if mode=='logistic':
        ...
        model = LogisticRegression(solver=solver, n_jobs=n_jobs, 
                                 max_iter=max_iter, random_state=666, multi_class='auto')
    elif mode=='Tree':
    ...
    elif mode=='RandomForestReg':
        n_estimators = kwargs.get('n_estimators', 50)
#         model = RandomForestRegressor(max_depth=4, n_estimators=n_estimators, random_state=666)
#         n_estimators = kwargs.get('n_estimators', 50)
        model = RandomForestRegressor(max_depth=100, n_estimators=n_estimators, random_state=666)#, min_samples_split = 0.05, min_samples_leaf = 0.001)
#         model = RandomForestClassifier(n_estimators=100, criterion = 'gini', max_features = None,  min_samples_split = 0.05, min_samples_leaf = 0.001)
    elif mode == 'mlpreg':
        model = MLPRegressor(hidden_layer_sizes=(10,100),  activation='relu', solver='adam', alpha=0.001,batch_size='auto',
        ...
        model = MLPClassifier(activation = 'logistic', solver='lbfgs', 
                    alpha=1e-4, hidden_layer_sizes=(6, 100), early_stopping=False,
                    max_iter= 5000,
        ...
        if mode=='NN':
            model = MLPClassifier(solver=solver, hidden_layer_sizes=hidden_layer_sizes,
                                ...
                                warm_start = warm_start, max_iter=max_iter, verbose=False,
                                early_stopping=early_stopping)
        if mode=='NN_reg':
...
#     print("beta\ty\t\t_true\tfunct")
#     print(beta,y[:10],y_true[:10],funct)
...
def error(mem):
    ...
    if len(mem) < 10: # why it uses 100? for the number of train size is equal to 100?
        return 1.0
    all_vals = (np.cumsum(mem, 0)/np.reshape(np.arange(1, len(mem)+1), (-1,1)))[-100:]
    errors = np.mean(np.abs(all_vals[-100:] - all_vals[-1:])/(np.abs(all_vals[-1:]) + 1e-12), -1) # confirm the variation of the shapley value converge towards to a small number(error)
